...
export default function readStudent() {
  ...
  useEffect(() => {
    ...
    const getStudent = async () => {
      try {
        ...
        setMessage({ message: "Estudante encontrado com sucesso! ", status: "ok" });  // Mensagem fixa de sucesso na busca
      } catch (error) {
  ...
  const handleUpdateStudent = async () => {
    try {
      console.log("Dados enviados para atualização:", student);  // Log para verificação
  
      const response = await Axios.put(API_URL + pid, student); // Enviando dados corretamente
          ...
          <div className="row border-bottom">
            ...
            <form method="POST"> 
                ...
                <div className="form-group p-2">
                    <button className={styles.btnUpdates} type="button" onClick={handleUpdateStudent}>Salvar</button>
                </div>
                </form>
          </div>
        </div>
